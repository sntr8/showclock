...
- **`ShowRemainingView` is a separate view, not inlined in `SettingsView`.** `qlab.totalRemainingSeconds` ticks roughly every second while connected (driven by the live poll cycle) — reading it directly in `SettingsView.body` would force the whole Form to re-render on that cadence.
