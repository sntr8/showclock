...
| QLab/QLabManager.swift | OSC UDP client; polls QLab every 1 s for the selected cue, `runningOrPausedCues` (drives `hasActiveCues`, used to end overtime early), and the full cue list + per-cue `type`/`duration`/`armed`/`actionElapsed`/parent `mode` (drives `totalRemainingSeconds` — the whole rest-of-show estimate, shown only in Settings; see docs/qlab-osc.md for the non-obvious "where does the sum start" logic and how Timeline-mode Groups — multiple tracks under one cue — are counted once instead of per track) |
...
- **Multiple tracks under one cue (Timeline groups).** `totalRemainingSeconds` used to sum every leaf cue's own `duration` independently, so a QLab Group cue in "Timeline" mode (all its children start together — how a show builds one "cue" out of several tracks, e.g. stereo stems) was counted once per track instead of once for the whole cue, inflating the estimate. Fixed by fetching each leaf's containing Group's `mode` (`/cue_id/{id}/mode`, cached forever like `type`/`duration`) and, when summing, treating consecutive leaves sharing a Timeline-mode parent as one cluster worth only its longest track's remaining time. See docs/qlab-osc.md.
