...
- **Each leaf's containing Group's `mode`**, fetched once and cached forever, via `/cue_id/{id}/mode` on the *parent* Group's ID (not the leaf itself). Needed to detect Timeline groups — see below.
...
**Timeline groups (multiple tracks per cue) count once, not per track.** QLab's Group cue `mode` property (`/cue_id/{id}/mode`, values `0`–`6`) is `3` for "Timeline" — the mode where every child cue starts simultaneously when the Group fires, which is how a show builds a single "cue" out of several audio tracks (e.g. stereo stems, a click track alongside a backing track). `buildCueOrder` flattens those children into consecutive leaves in `cueOrder`, and each one individually reports its own full track length via `duration` — so naively summing every leaf would multiply a single cue's length by its track count. `recomputeTotalRemaining` instead groups consecutive leaves that share the same immediate parent (`parentGroupID`) when that parent's cached `mode == 3`, and adds only the longest remaining track in the cluster, since they all end around the same time. Leaves under any other mode (List, Playlist, Start First, etc.) are summed individually as before, since those genuinely play one at a time. Cues that fire simultaneously *without* being grouped this way (e.g. two independent, unrelated cues an operator has scheduled to overlap) still each count in full — the fix only recognizes explicit Timeline groups, so a heavily-overlapping show built without them will still over-estimate somewhat.
